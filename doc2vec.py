...
try:
    ...
    def train_document_dbow(model, doc_words, doctag_indexes, alpha, work=None, 
                            ...
                            word_vectors=None, word_locks=None, doctag_vectors=None, doctag_locks=None, compute_loss=False):
        ...
        for doctag_index in doctag_indexes:
            for word in doc_words:
                train_sg_pair(
                    ...
                    context_vectors=doctag_vectors, context_locks=doctag_locks, compute_loss=compute_loss
                )
    ...
    def train_document_dm(model, doc_words, doctag_indexes, alpha, work=None, neu1=None,
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            neu1e = train_cbow_pair(model, word, word2_indexes, l1, alpha,
                                    learn_vectors=False, learn_hidden=learn_hidden, compute_loss=compute_loss)
    ...
    def train_document_dm_concat(model, doc_words, doctag_indexes, alpha, work=None, neu1=None, learn_doctags=True,
                                 ...
                                 doctag_vectors=None, doctag_locks=None, compute_loss=False):
        ...
        for pos in range(pre_pad_count, len(padded_document_indexes) - post_pad_count):
            ...
            neu1e = train_cbow_pair(model, predict_word, None, l1, alpha,
                                    learn_hidden=learn_hidden, learn_vectors=False, compute_loss=compute_loss)
...
try:
    ...
    def d2v_train_epoch_dbow(model, corpus_file, offset, start_doctag, _cython_vocab, _cur_epoch, _expected_examples,
                             _expected_words, work, _neu1, docvecs_count, compute_loss, word_vectors=None, word_locks=None,
                             train_words=False, learn_doctags=True, learn_words=True, learn_hidden=True,
    ...
    def d2v_train_epoch_dm_concat(model, corpus_file, offset, start_doctag, _cython_vocab, _cur_epoch,
                                  _expected_examples, _expected_words, work, _neu1, docvecs_count, compute_loss, word_vectors=None,
...
class Doc2Vec(BaseWordEmbeddingsModel):
        ...
        for doc in job:
            ...
            if self.sg:
                tally += train_document_dbow(
                    ...
                    doctag_vectors=doctag_vectors, doctag_locks=doctag_locks, compute_loss=self.compute_loss
                )
            elif self.dm_concat:
    ...
    def train(self, documents=None, corpus_file=None, total_examples=None, total_words=None,
              epochs=None, start_alpha=None, end_alpha=None, compute_loss=False,
        ...
        super(Doc2Vec, self).train(
            ...
            queue_factor=queue_factor, report_delay=report_delay, compute_loss=compute_loss, callbacks=callbacks, **kwargs)
        ...
        for i in range(epochs):
            if self.sg:
                train_document_dbow(
                    ...
                    learn_words=False, learn_hidden=False, doctag_vectors=doctag_vectors, doctag_locks=doctag_locks, compute_loss=self.compute_loss
                )
            elif self.dm_concat:
